...
export function LandlordPropertyForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      categoryId:
        initialProperty?.categoryId ??
        initialProperty?.category?.id ??
        (categories[0]?.id || ""),
  ...
  function addImage() {
    ...
    if (
      ...
      toast.error("Invalid URL", {
        description: "Image link must start with http://, https://, or /",
      })
      return
    }
  ...
  async function onSubmit(values: PropertyFormValues) {
    const payload: CreatePropertyPayload = {
      ...
      bedrooms: values.bedrooms == null ? undefined : Number(values.bedrooms),
      bathrooms:
        values.bathrooms == null ? undefined : Number(values.bathrooms),
      size: values.size == null ? undefined : Number(values.size),
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Provide accurate details and photos to showcase your property to
            prospective tenants.
          </p>
        </div>
      </div>
      ...
      <Card className="bg-card/90">
        <CardHeader>
          ...
          <CardDescription>
            Title, location, category, and pricing.
          </CardDescription>
        </CardHeader>
      ...
      <Card className="bg-card/90">
        <CardHeader>
          ...
          <CardDescription>
            Number of rooms, floor area, and amenities.
          </CardDescription>
        </CardHeader>
      ...
      <Card className="bg-card/90">
        <CardHeader>
          ...
          <CardDescription>
            Add public image URLs for your property listing.
          </CardDescription>
        </CardHeader>
          ...
          {images.length > 0 ? (
            <div className="grid gap-3 pt-2 sm:grid-cols-2 lg:grid-cols-3">
              {images.map((url, idx) => (
                <div
                  key={`${url}-${idx}`}
                  className="group relative flex items-center justify-between rounded-lg border bg-muted/30 p-2 text-xs"
                >
            ...
            <p className="text-xs text-muted-foreground italic">
              No custom image URLs added yet. A placeholder image will be used
              if left blank.
            </p>
          )}
        </CardContent>
